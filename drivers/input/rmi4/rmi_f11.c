...
#define F11_DEFAULT_LED_DIFF 5	//F11_DEFAULT_LED_HI - #define F11_DEFAULT_LED_LOW
#else
...
void turnoff_keypad_bl(struct work_struct *work){
	led_trigger_event(keypad_led_trigger, basic_led_level);
}
...
static void rmi_early_suspend(struct early_suspend *handler)
{
	...
	led_trigger_event(keypad_led_trigger, 0);
	unlock1time = 0;
...
static void rmi_late_resume(struct early_suspend *handler)
{
	...
	if ( g_bIsPadAttach == false ){
			...
			rmi_f11_resume(g_fc);
		#endif /* RESUME_REZERO */
		led_trigger_event(keypad_led_trigger, finger_moving_led_level);
		if(delayed_work_pending(&keypad_off_work)) {
			cancel_delayed_work_sync(&keypad_off_work);
		}
...
//ASUS_BSP simpson: add for finger moving trigger +++
		if ((i == (sensor->nbr_fingers - 1)) && (finger_moving_led_level > 0)){
...
{
	char *virtual_keys = 	__stringify(EV_KEY) ":" __stringify(KEY_BACK) ":100:1340:200:115" "\n" \
				__stringify(EV_KEY) ":" __stringify(KEY_HOME) ":360:1340:300:115" "\n" \
				__stringify(EV_KEY) ":" __stringify(KEY_MENU)   ":620:1340:200:115" "\n" ;
...
INIT_DELAYED_WORK(&keypad_off_work, turnoff_keypad_bl);
	led_trigger_event(keypad_led_trigger, key_pressed_led_level);
	queue_delayed_work(keypad_bl_workqueue, &keypad_off_work, msecs_to_jiffies(user_duration));
//ASUS_BSP simpson: add for keypad_bl ---
//ASUS_BSP simpson: add for suspend/resume +++
#ifdef CONFIG_HAS_EARLYSUSPEND
				    ...
				    size_t count)
{
	...
	//force update! ++
	if(!delayed_work_pending(&keypad_off_work)) led_trigger_event(keypad_led_trigger, basic_led_level); //not pending means stay basic_led_level
	//force update! --
				    ...
				    size_t count)
{
	if (sscanf(buf, "%d", &user_mode) != 1)
		return -EINVAL;
	if (key_pressed_led_level == 0) key_pressed_led_level = F11_DEFAULT_LED_HI;
	if (finger_moving_led_level == 0) finger_moving_led_level = F11_DEFAULT_LED_LOW;
	if (user_mode >= (F11_LED_MAX_TIME*-1) && user_mode <= -100) {	//manually setting turn on duration(100ms~F11_LED_MAX_TIME) with basic_led_level=1
		...
		key_pressed_led_level = F11_DEFAULT_LED_BASIC;
		finger_moving_led_level = key_pressed_led_level;
		basic_led_level = finger_moving_led_level;
