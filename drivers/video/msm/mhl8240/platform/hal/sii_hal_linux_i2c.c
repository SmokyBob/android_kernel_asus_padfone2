...
//	printk("[MHL] i2c_transfer (0x%x) (0x%x)--- \n", pMsgs->addr, *pMsgs->pBuf);
    	...
    	if(i2cStatus < msgCount)
    	{
    		...
    		if (i2cStatus < 0)
    		{
    	    	...
    	    	printk( "SiiMasterI2cTransfer, i2c_transfer error: %d  " \
    	    			"deviceId: 0x%02x regOffset: 0x%02x\n",
    	    			i2cStatus, pMsgs->addr, *pMsgs->pBuf);
...
//ASUS_BSP +++ 			
		...
		{
			MHL_disable_irq();
			printk("[MHL i2c error -5] %d mode , disable irq\n", fwPowerState);
			ASUSEvtlog("[MHL] i2c error -5 mode=(%d), disable irq\n", fwPowerState);
		}
		...
		if (i2cStatus == -107)
		{
			...
			{
				MHL_disable_irq();
				printk("[MHL i2c error -107] %d mode , disable irq\n", fwPowerState);	
				ASUSEvtlog("[MHL] i2c error -107 mode=(%d) , disable irq\n", fwPowerState);
