...
//if hdcp on, then stand-by , tmds off will re-trigger TpiIsr
		    ...
		    if ( ((H_RES_L == 0x5a) && (H_RES_H == 0x03) 
				...
				&& (V_RES_L  == 0x65) && (V_RES_H== 0x04))    // 1080p30   1125x2200
				)	
...
#ifdef CP9687_CHECK_DEVCAP//20121004 hammertest   avoid EDID read before read devcap complete
{
uint8_t int9Status;
HalTimerWait(20);
    int9Status = SiiRegRead(REG_INTR9);
    ...
    else if (int9Status)
    {
        ...
        {
                     mscCmdInProgress = false;
            ERROR_DEBUG_PRINT(("BIT_INTR9_DEVCAP_DONE ok!!:\n"));
            ...
            SiiMhlTxMscCommandDone(0);
                     ...
                     {
                     ERROR_DEBUG_PRINT(("read dev_cap fail?:\n"));
                     }
}
#endif			
            break;
...
//ASUS_BSP , check carkit cable in/out in MHL and USB driver +++
