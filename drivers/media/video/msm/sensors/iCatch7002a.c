...
#include <linux/pm_qos_params.h> //ASUS_BSP LiJen "[A68][ISP][NA][Fix] vote don't enter low power mode when ISP flash"
...
struct pm_qos_request_list pm_qos_req_list; //ASUS_BSP LiJen "[A68][ISP][NA][Fix] vote don't enter low power mode when ISP flash"
...
int sensor_set_mode(int  res)
{
       ...
       switch(res){
		  ...
		  pr_info("%s: MODE_5\n",__func__);
            ...
            case MSM_SENSOR_RES_FULL_SINGLE_CAPTURE:    //MODE_7
		      ...
		      sensor_write_reg(imx091_s_ctrl.sensor_i2c_client->client, 0x7127, 0x20);//HDR posotive +2.0EV
		      sensor_write_reg(imx091_s_ctrl.sensor_i2c_client->client, 0x7128, 0x20);//HDR nagative +2.0EV
       ...
       if(true == iCatch_first_open){                  
            ...
            iCatch_set_led_mode(LED_MODE_OFF); //LiJen: "[A68][ISP][NA][Fix]workaround: disable flash to avoid CTS take picture timeout"
       }else{
       	wait_for_next_frame();	
       }
